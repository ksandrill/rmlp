...
def main():
    ...
    w1, w2, w3, _, _ = load_model(parameters_file_name)
    input_size, r_fc1_size = unpack_size(w1, is_rec=True)
    _, r_fc2_size = unpack_size(w2, is_rec=True)
    _, output_size = unpack_size(w3, is_rec=False)
    rmlp = Rmlp(input_size, r_fc1_size, r_fc2_size, output_size, 1, 0)
    rmlp.load_weights(w1, w2, w3)
