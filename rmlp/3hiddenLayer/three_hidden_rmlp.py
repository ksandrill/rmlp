...
Outputs: type = list[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
OutputsWithoutActiv: type = list[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
Inputs: type = list[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
DataSet: type = list[(np.ndarray, np.ndarray)]
Gradients: type = list[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
LoopOutGrads: type = list[np.ndarray, np.ndarray, np.ndarray]


class ThreeHiddenRmlp:
    def __init__(self, input_size: int, first_layer_size: int, second_layer_size: int, third_layer_size: int,
        ...
        self.r_fc3 = RecFcLayer(second_layer_size, third_layer_size, bias=True, low=low, high=high)
        self.output_fc = FcLayer(third_layer_size, output_layer_size, bias=True, low=low, high=high)

    def forward(self, input_data: np.ndarray, prev_outputs: Outputs) -> (Inputs, OutputsWithoutActiv, Outputs):
        out1, out2, out3, _ = prev_outputs
        pre_out1, input1 = self.r_fc1.forward(input_data, out1)
        out1 = np.tanh(pre_out1)
        pre_out2, input2 = self.r_fc2.forward(out1, out2)
        out2 = np.tanh(pre_out2)
        pre_out3, input3 = self.r_fc3.forward(out2, out3)
        out3 = np.tanh(pre_out3)
        pre_out4, input4 = self.output_fc.forward(out3)
        out4 = np.tanh(pre_out4)
        return [input1, input2, input3, input4], [pre_out1, pre_out2, pre_out3, pre_out4], [out1, out2, out3, out4]
    ...
    def _update_weights(self, gradients: Gradients, lr: float):
        ...
        self.r_fc3.layer_weighs -= lr * gradients[2]
        self.output_fc.layer_weighs -= lr * gradients[3]

    def load_weights(self, w1: np.ndarray, w2: np.ndarray, w3: np.ndarray, w4: np.ndarray):
        self.r_fc1.layer_weighs = w1
        self.r_fc2.layer_weighs = w2
        self.r_fc3.layer_weighs = w3
        self.output_fc.layer_weighs = w4
    ...
    def save_data(self, parameters_file_name, lr, epoch_count=0):
        with open(parameters_file_name, 'wb') as parameters_file:
            ...
            w3 = self.r_fc3.layer_weighs
            w4 = self.output_fc.layer_weighs
            pickle.dump([w1, w2, w3, w4, lr, epoch_count], parameters_file)
    ...
    def fit(self, lr, epoch_count, train_set: DataSet, parameters_file_name: str) -> list[float]:
        ...
        for i in range(epoch_count):
            ...
            prev_outputs: Outputs = [np.zeros(self.r_fc1.output_features_size),
                                     ...
                                     np.zeros(self.r_fc3.output_features_size),
                                     np.zeros(self.output_fc.output_features_size)]
            prev_grad_out3 = np.zeros_like(self.r_fc3.layer_weighs)
            ...
            grads: Gradients = [np.zeros_like(self.r_fc1.layer_weighs), np.zeros_like(self.r_fc2.layer_weighs),
                                np.zeros_like(self.r_fc3.layer_weighs),
                                np.zeros_like(self.output_fc.layer_weighs)]
            avg_mse = 0.0

            for item in train_set:
                x, y = item
                [input1, input2, input3, input4], [v1, v2, v3, v4], prev_outputs = self.forward(x, prev_outputs)
                error = prev_outputs[-1] - y
                w4_grad, error = self.output_fc.grad(error, input4, v4)
                w3_grad, prev_grad_out3, error = self.r_fc3.grad(error, input3, prev_grad_out3, v3)
